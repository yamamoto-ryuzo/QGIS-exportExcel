...
def get_exp_variable_for_feature(exp: QgsExpression, feature: QgsFeature):
    """
    指定の式に地物を設定し、評価した結果を返却する
...
def output_single_report(excel_app, template_path: str, feature: QgsFeature, output_path: str, dpi: int, progress: QProgressDialog):
    """
    ...
    if progress.wasCanceled():
        cancel_return = QMessageBox.question(iface.mainWindow(), "確認", "処理を中止しますか。", QMessageBox.Ok, QMessageBox.Cancel)
        if cancel_return == QMessageBox.Ok:
            return False
        progress.reset()
...
def get_raw_variable_for_layer(layer: QgsVectorLayer):
    """
    レイヤ変数からテンプレートファイルパス、出力ファイルパスを取得し、設定値を辞書にして返却する

      Parameters
        layer -- レイヤ

      Returns
        テンプレートファイルパス、出力ファイルパスのレイヤ変数定義値の辞書
    """

    raw = {}
    template_path = get_layer_variable(TEMPLATE_PATH, layer)
    if not template_path:
        iface.messageBar().pushCritical("ERROR", f"レイヤ変数'template_path'が未設定です")
        return None
    output_path_fixed = get_layer_variable(OUTPUT_PATH_FIXED, layer)
    if not output_path_fixed:
        iface.messageBar().pushCritical("ERROR", f"レイヤ変数'output_path'が未設定です")
        return None
    output_path_variable = get_layer_variable(OUTPUT_PATH_VARIABLE, layer)
    if not output_path_variable:
        iface.messageBar().pushCritical("ERROR", f"レイヤ変数'output_path_variable'が未設定です")
        return None

    raw[TEMPLATE_PATH] = template_path
    raw[OUTPUT_PATH_FIXED] = output_path_fixed
    raw[OUTPUT_PATH_VARIABLE] = output_path_variable

    return raw

def get_exp_variable_for_layer(raw: dict):
    """
    テンプレートファイルパス、出力ファイルパスの辞書から、式として成り立つものを抽出して辞書にして返却する

      Parameters
        raw -- テンプレートファイルパス、出力ファイルパスの辞書
    ...
    exp = {}
    template_path = create_expression(raw.get(TEMPLATE_PATH))
    if template_path:
        exp[TEMPLATE_PATH] = template_path
    output_path_fixed = create_expression(raw.get(OUTPUT_PATH_FIXED))
    if output_path_fixed:
        exp[OUTPUT_PATH_FIXED] = output_path_fixed
    output_path_variable = create_expression(raw.get(OUTPUT_PATH_VARIABLE))
    if output_path_variable:
        exp[OUTPUT_PATH_VARIABLE] = output_path_variable

    return exp

def get_variable_for_feature(feature: QgsFeature, raw: dict, exp: dict, variable_name: str):
    """
    レイヤ変数からテンプレートファイルパス、出力ファイルパスを抽出し、文字列あるいは式を評価した結果を返却する

      Paramters
        feature -- 地物
        raw -- レイヤ変数の定義値の辞書
        exp -- レイヤ変数の中で式となるものの辞書
        variable_name -- レイヤ変数名

      Returns
        文字列あるいは式を評価した結果
    """

    if not variable_name in exp:
        return raw.get(variable_name)
    
    target_exp = exp.get(variable_name)

    context = QgsExpressionContext()
    context.setFeature(feature)
    return target_exp.evaluate(context)


# マップキャンバスの現在のカーソルを退避し、砂時計にする
map_canvas = iface.mapCanvas()
save_cursor = map_canvas.cursor()
map_canvas.setCursor(Qt.WaitCursor)

while True:
    # 対象レイヤーの全ての地物に関して帳票を出力する
    ...
    # レイヤー変数からテンプレートパス、出力パス、dpiを取得作成する
    ...
    raw = get_raw_variable_for_layer(layer)
    if not raw:
        break

    # 地物の属性値に基づいて値を変える
    exp = get_exp_variable_for_layer(raw)

    # 全ての地物に対する出力先フォルダ（ユーザー指定部）の存在チェック
    output_path_fixed_for_all = ""
    if OUTPUT_PATH_FIXED in exp:
        flg_check_output_folder = True
        ...
        if not flg_check_output_folder:
            break
        ...
        if not os.path.isdir(path_):
            iface.messageBar().pushCritical("ERROR", f"出力先フォルダ\n {path_} \nは存在していません。")
            break
        output_path_fixed_for_all = path_

    msg_return = QMessageBox.question(iface.mainWindow(), "確認", f'{featureCount}件の帳票出力を開始します。\n時間がかかりますが処理を続行しますか。', QMessageBox.Ok, QMessageBox.Cancel)
    if msg_return == QMessageBox.Cancel:
        break
    ...
    # 進捗ダイアログ表示
    progress = QProgressDialog("帳票出力中...", "キャンセル", 0, featureCount, iface.mainWindow())
    progress.setAutoClose(True)
    progress.setValue(0)

    # map_canvas.freeze(True)

    # 地物をfidの順に処理するためfidのリストを作成
    fids = []
    for f in features:
        fids.append(f.id())
    # ソート
